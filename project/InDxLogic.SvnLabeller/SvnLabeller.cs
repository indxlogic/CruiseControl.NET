...
namespace InDxLogic.SvnLabeller
{
    ...
    {
        #region Fields
        private const string MAJOR_TOKEN = "(major)";
        private const string MINOR_TOKEN = "(minor)";
        private const string BUILD_TOKEN = "(build)";
        private const string REVISION_TOKEN = "(revision)";
        private const string REBUILD_TOKEN = "(rebuild)";
        private const string DATE_TOKEN = "(date)";
        private const string MS_REVISION_TOKEN = "(msrevision)";
        private const string REVISION_X_PATH = "/log/logentry/@revision";
        ...
        public string Generate(IIntegrationResult resultFromLastBuild)
        {
            ...
            int elapsedDays = CalculateElapsedDays(StartDate, systemClock.Now);
            int msRevision = CalculateMsRevision(systemClock.Today, systemClock.Now);

            // Replace the tokens in the pattern with the appropriate string formatting placeholders
            string format = Pattern.Replace(MAJOR_TOKEN, "{0}")
                .Replace(MINOR_TOKEN, "{1}")
                .Replace(BUILD_TOKEN, "{2}")
                .Replace(REVISION_TOKEN, "{3}")
                .Replace(REBUILD_TOKEN, "{4}")
                .Replace(DATE_TOKEN, "{5}")
                .Replace(MS_REVISION_TOKEN, "{6}");

            string label = string.Format(format, Major, Minor, build, revision, rebuild, elapsedDays, msRevision);
        ...
        private static int CalculateMsRevision(DateTime startDate, DateTime currentDate) => (int)((currentDate - startDate).TotalSeconds / 2);
        ...
        protected virtual int GetRevision()
        {
            ...
            try
            {
                ...
                XmlNode node = xml.SelectSingleNode(REVISION_X_PATH);
        ...
        protected virtual string Quote(string urlToBeQuoted) => $@"""{urlToBeQuoted}""";
        ...
        protected virtual Version ParseVersion(int revision, IIntegrationResult resultFromLastBuild)
        {
            ...
            if (!String.IsNullOrEmpty(Pattern))
            {
                ...
                pattern = Pattern
                    .Replace(MAJOR_TOKEN, "(?<major>[0-9]+)")
                    .Replace(MINOR_TOKEN, "(?<minor>[0-9]+)")
                    .Replace(BUILD_TOKEN, "(?<build>[0-9]+)")
                    .Replace(REVISION_TOKEN, "(?<revision>[0-9]+)")
                    .Replace(REBUILD_TOKEN, "(?<rebuild>[0-9]+)");
            }
            ...
            try
            {
                ...
                else
                {
                    ...
                    Int32.TryParse(match.Groups["rebuild"].Value, out rebuild);
